...
def _latest_from_db(page, limit, category=None):
    db = SessionLocal()
    try:
        q = db.query(Post)
        if category is not None:
            q = q.filter(Post.category == int(category))
        total = q.count()
...
def latest():
    ...
    category = request.args.get("category")  # 分区过滤（可选，1-4）
    if category is not None:
    ...
    posts, total = _latest_from_db(page, limit, category=category)

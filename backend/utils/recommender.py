...
# 源平台分区名 → 目标分区 id 的直接映射（结构化强信号，优先于关键词）
SOURCE_CATEGORY_MAP = {
    "二手闲置": 2, "收笔记": 2, "求购": 2, "转让": 2, "出售": 2,
    "失物招领": 3, "寻物": 3, "失物": 3,
    "组队": 4, "组队活动": 4, "比赛": 4, "赛事": 4, "招募": 4, "活动": 4,
}


def classify(text, source_name=None):
    """返回分区 id (1-4)。source_name 为源平台分区名，命中直映射时优先归类。"""
    if source_name:
        for key, cat in SOURCE_CATEGORY_MAP.items():
            if key in source_name:
                return cat
    # 源分区名仍作为关键词匹配的上下文（与旧逻辑一致），text 为正文
    ...
    for c, kws in CATEGORY_KEYWORDS.items():
        ...
        scores[c] = sum(1 for kw in kws if kw in blob) + sum(1 for kw in kws if kw in tokens)
